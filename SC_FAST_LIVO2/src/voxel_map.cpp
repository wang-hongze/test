...
std::vector<VoxelPlane*> VoxelMapManager::getVoxelsInRange(double x, double y, double z, double radius)
{
    std::vector<VoxelPlane*> result;
    if (voxel_map_.empty()) return result;

    // 体素大小（由配置参数获取）
    double voxel_size = config_setting_.max_voxel_size_;
    if (voxel_size <= 0) {
        ROS_WARN("[VoxelMapManager] Invalid voxel size: %f", voxel_size);
        return result;
    }

    // 计算中心位置对应的体素索引
    int64_t center_idx_x = static_cast<int64_t>(floor(x / voxel_size));
    int64_t center_idx_y = static_cast<int64_t>(floor(y / voxel_size));
    int64_t center_idx_z = static_cast<int64_t>(floor(z / voxel_size));

    // 计算半径覆盖的体素索引范围（向上取整确保覆盖完整范围）
    int range_idx = static_cast<int>(ceil(radius / voxel_size)) + 1; // +1 防止边界遗漏

    // 遍历范围内的所有体素索引
    for (int64_t dx = -range_idx; dx <= range_idx; ++dx) {
        for (int64_t dy = -range_idx; dy <= range_idx; ++dy) {
            for (int64_t dz = -range_idx; dz <= range_idx; ++dz) {
                // 计算当前体素索引
                VOXEL_LOCATION curr_loc(
                    center_idx_x + dx,
                    center_idx_y + dy,
                    center_idx_z + dz
                );
                ...
                // 检查该体素是否存在于地图中
                auto it = voxel_map_.find(curr_loc);
                if (it == voxel_map_.end()) continue;

                VoxelOctoTree* octree = it->second;
                if (!octree) continue;

                // 若体素包含有效平面，添加到结果中
                if (octree->plane_ptr_ && octree->plane_ptr_->is_plane_) {
                    // 额外检查平面中心是否在半径范围内（精确过滤）
                    Eigen::Vector3d plane_center = octree->plane_ptr_->center_;
                    double dist = sqrt(
                        pow(plane_center.x() - x, 2) +
                        pow(plane_center.y() - y, 2) +
                        pow(plane_center.z() - z, 2)
                    );
                    if (dist <= radius) {
                        result.push_back(octree->plane_ptr_);
                    }
                }
            }
        }
...
// 新增：将Pose6D转换为旋转矩阵和平移向量
std::pair<Eigen::Matrix3d, Eigen::Vector3d> pose6DToEigen(const Pose6D& pose)
{
    // 从roll/pitch/yaw计算旋转矩阵（X-Y-Z欧拉角）
    ...
    Eigen::Matrix3d R = (yawAngle * pitchAngle * rollAngle).toRotationMatrix(); // 注意乘法顺序
...
void VoxelOctoTree::correctPose(const Eigen::Matrix3d& R_correction, const Eigen::Vector3d& t_correction)
{
  // 校正当前节点的中心坐标
  Eigen::Vector3d center(voxel_center_[0], voxel_center_[1], voxel_center_[2]);
  center = R_correction * center + t_correction;
  voxel_center_[0] = center.x();
  voxel_center_[1] = center.y();
  voxel_center_[2] = center.z();

  // 校正平面参数（若为平面体素）
  if (plane_ptr_ && plane_ptr_->is_plane_) {
    plane_ptr_->center_ = R_correction * plane_ptr_->center_ + t_correction;
    plane_ptr_->normal_ = R_correction * plane_ptr_->normal_;
    plane_ptr_->x_normal_ = R_correction * plane_ptr_->x_normal_;
    plane_ptr_->y_normal_ = R_correction * plane_ptr_->y_normal_;
    plane_ptr_->d_ = -(plane_ptr_->normal_.dot(plane_ptr_->center_));
  }

  // 递归校正子节点（若为非叶子节点）
  if (octo_state_ == 1) {  // octo_state_=1 表示非叶子节点
    for (int i = 0; i < 8; ++i) {
      if (leaves_[i] != nullptr) {
        leaves_[i]->correctPose(R_correction, t_correction);
      }
    }
  }
}
...
// 在voxel_map.cpp中添加
#include <Eigen/Geometry>  // 确保在本文件顶部已包含

void VoxelMapManager::updateWithOptimizedPoses(
    const std::vector<std::pair<Eigen::Matrix3d, Eigen::Vector3d>>& optimized_poses)
{
  std::lock_guard<std::mutex> lock(voxel_map_mutex_);  // 线程安全保护

  if (optimized_poses.empty()) {
    ROS_WARN("[VoxelMapManager] Optimized poses is empty");
    return;
  }
  if (optimized_poses.size() != original_keyframe_poses_.size()) {
    ROS_WARN("[VoxelMapManager] Optimized poses size (%zu) does not match original keyframes (%zu)",
             optimized_poses.size(), original_keyframe_poses_.size());
    return;
  }

  for (size_t kf_idx = 0; kf_idx < optimized_poses.size(); ++kf_idx) {
    // 原始位姿（roll/pitch/yaw -> 旋转矩阵；xyz -> 平移）
    const Pose6D& original_pose = original_keyframe_poses_[kf_idx];
    Eigen::Matrix3d R_original =
        Eigen::AngleAxisd(original_pose.yaw,   Eigen::Vector3d::UnitZ()).toRotationMatrix() *
        Eigen::AngleAxisd(original_pose.pitch, Eigen::Vector3d::UnitY()).toRotationMatrix() *
        Eigen::AngleAxisd(original_pose.roll,  Eigen::Vector3d::UnitX()).toRotationMatrix();
    Eigen::Vector3d t_original(original_pose.x, original_pose.y, original_pose.z);

    // C++14 下不要用结构化绑定
    const Eigen::Matrix3d& R_optimized = optimized_poses[kf_idx].first;
    const Eigen::Vector3d& t_optimized = optimized_poses[kf_idx].second;

    // T_corr = T_opt * T_org^{-1}
    Eigen::Matrix3d R_correction = R_optimized * R_original.transpose();
    Eigen::Vector3d t_correction = t_optimized - R_correction * t_original;

    // 取出该关键帧关联的体素并应用校正
    auto voxel_it = voxel_keyframe_map_.find(kf_idx);
    if (voxel_it == voxel_keyframe_map_.end() || voxel_it->second.empty()) {
      ROS_DEBUG("[VoxelMapManager] No voxels associated with keyframe %zu", kf_idx);
      continue;
    }

    for (const auto& voxel_loc : voxel_it->second) {
      auto map_it = voxel_map_.find(voxel_loc);
      if (map_it == voxel_map_.end() || !map_it->second) {
        ROS_DEBUG("[VoxelMapManager] Voxel at (%lld, %lld, %lld) not found",
                  (long long)voxel_loc.x, (long long)voxel_loc.y, (long long)voxel_loc.z);
        continue;
      }
      map_it->second->correctPose(R_correction, t_correction);
    }
  }

  ROS_INFO("[PGO] Voxel map updated with %zu optimized poses", optimized_poses.size());
}
